package rulite

import (
	"fmt"
	"strings"
)

// Explanation is an immutable view of Result's canonical ledger. It is
// available without Trace and never reevaluates conditions. Structural
// semantics are supported in v0.x; String's exact formatting may change.
type Explanation struct{ result Result }

// Rules returns independent execution views for every rule in compiled order.
func (e Explanation) Rules() []RuleExecution {
	if e.result.metadata == nil {
		return nil
	}
	rules := make([]RuleExecution, e.result.counts.Total)
	for order := range rules {
		rules[order] = e.result.ruleAt(order)
	}
	return rules
}

// String describes the global stop, counts, and each top-level rule or group
// followed by its members in compiled local order, including empty groups.
// It does not include input, action parameters, panic values, or stacks.
// Business error messages are included as supplied by the callback.
// The text is intended for people, not parsing or character-level compatibility.
func (e Explanation) String() string {
	if !e.result.Executed() {
		return "execution: not-started\nstop reason: none\n"
	}
	var text strings.Builder
	r := e.result
	fmt.Fprintf(&text, "execution: %s\n", r.stop)
	c := r.counts
	fmt.Fprintf(&text, "summary: total=%d evaluated=%d not-evaluated=%d matched=%d unmatched=%d fired=%d skipped=%d failed=%d condition-failed=%d action-failed=%d panic-recovered=%d\n",
		c.Total, c.Evaluated, c.NotEvaluated, c.Matched, c.Unmatched, c.Fired, c.Skipped, c.Failed, c.ConditionFailed, c.ActionFailed, c.PanicRecovered)
	for order := 0; order < e.entryCount(); order++ {
		entry := e.entryAt(order)
		if group, ok := entry.Group(); ok {
			selected, _ := group.SelectedRule()
			fmt.Fprintf(&text, "\ngroup %s kind=%s top-level-order=%d registration-index=%d priority=%d state=%s selected-rule=%s end-reason=%s stop=%s\n", group.ID(), group.Kind(), group.Order(), group.RegistrationIndex(), group.Priority(), group.State(), selected, group.EndReason(), group.StopReason())
			for member := entry.entry.start; member < entry.entry.end; member++ {
				writeRuleExplanation(&text, r.ruleAt(member), "  ", true)
			}
		} else {
			rule, _ := entry.Rule()
			writeRuleExplanation(&text, rule, "", len(r.metadata.groups) > 0)
		}
	}
	return text.String()
}

func writeRuleExplanation(text *strings.Builder, x RuleExecution, indent string, hasGroups bool) {
	fmt.Fprintf(text, "\n%srule %s order=%d registration-index=%d priority=%d\n", indent, x.ID(), x.Order(), x.RegistrationIndex(), x.Priority())
	if id, ok := x.GroupID(); ok {
		member, _ := x.MemberIndex()
		fmt.Fprintf(text, "%s  group=%s top-level-order=%d member-index=%d\n", indent, id, x.TopLevelOrder(), member)
	} else if hasGroups {
		fmt.Fprintf(text, "%s  top-level-order=%d\n", indent, x.TopLevelOrder())
	}
	fmt.Fprintf(text, "%s  evaluated=%t matched=%t action-started=%t action-success=%t state=%s\n", indent, x.Evaluated(), x.Matched(), x.ActionStarted(), x.Fired(), ruleStateText(x.State()))
	if failure, ok := x.Error().(Failure); ok {
		fmt.Fprintf(text, "%s  failure: %s; continued=%t\n", indent, failure.Error(), failure.Continued())
	}
	if x.SkipReason() == SkipContextDone {
		fmt.Fprintf(text, "%s  skip reason: context-done\n", indent)
	}
	if x.NotEvaluatedReason() == NotEvaluatedExecutionStopped {
		fmt.Fprintf(text, "%s  not-evaluated reason: execution-stopped\n", indent)
	}
	if x.NotEvaluatedReason() == NotEvaluatedGroupResolved {
		fmt.Fprintf(text, "%s  not-evaluated reason: group-resolved\n", indent)
	}
}

func ruleStateText(state RuleState) string {
	switch state {
	case RuleNotEvaluated:
		return "not-evaluated"
	case RuleUnmatched:
		return "unmatched"
	case RuleFired:
		return "fired"
	case RuleFailed:
		return "failed"
	case RuleSkipped:
		return "skipped"
	default:
		return "unknown"
	}
}

...
[`Runtime[T]`](runtime.md) publishes complete immutable engines, including their validated defaults. Fire loads one publication at entry and reuses the Engine state machine until completion. Publishers serialize revision assignment with the atomic store; each success increments from one, and failures preserve the current publication. Engine and RuleSet remain immutable.

`RuleSet.WithIdentity(version, digest)` creates an independent wrapper over compiled nodes with opaque caller-supplied identity strings. `SnapshotInfo` contains only business version, local publication revision, and optional source digest. Result, Explanation, Trace, and all Observer events expose the captured identity through `Snapshot()`. Direct Engine execution uses revision zero. Runtime holds no old-publication history, and retained diagnostic views have no executable references. Source acquisition and external resource lifetime remain caller responsibilities; Runtime has no background work or Close.
...
Delivery is synchronous on the Fire call and receives the caller's context. Implementations manage latency, backpressure, and synchronization across concurrent executions. Observer latency is part of Fire wall time and overall Trace duration, including finish delivery, but never Condition/Action durations. Core events contain no timings and their delivery reads no duration clock. An observer may independently measure duration for an explicit consumer. Observation failure isolation assumes the observer does not change context or external business state and excludes timing-sensitive external behavior. Real cancellation is checked at the existing next context boundary; no extra check separates evaluated(true) and matched. Cancellation during finish occurs after terminal business facts and cannot change them retroactively. Fire waits for a blocked observer to return even after cancellation. See the [read-only collector example](../observer_example_test.go).
...
[Dynamic definitions](dynamic-rules.md) compile strict JSON configuration through an explicit frozen typed action registry into ordinary rule sets. Decoding, parameter validation and capability resolution happen before Fire. Nested action parameters are shared read-only; providers synchronize captures and copy parameter data before placing it in mutable input. Configuration cannot discover arbitrary executable code. Compiled engines can be atomically published through [Runtime](runtime.md). The separate [OpenTelemetry adapter](observability.md) installs a private Observer per Fire, derives completion metrics from final events, and exports bounded ordered rule events under one execution span. It keeps root dependencies unchanged, replaces the existing observer slot, and leaves provider resource ownership with the caller.

The [CEL Condition adapter](cel.md) compiles expressions before execution, requires an exact boolean result type, and maps evaluation errors and unknown outcomes to ordinary Condition errors. It supports explicit typed projectors, native exported Go names or opt-in JSON field names, protobuf descriptors and presence, and trusted unary scalar functions. Builder registration freezes into an immutable compiler; programs use independent evaluation state. Its dependency, reflection, and resource limits stay in the integration package; root callback signatures and the standard-library runtime boundary remain unchanged. Input size and CEL work budgets remain cooperative at Go callback boundaries. CEL actions are outside the scope.

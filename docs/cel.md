...
In `v0.1.0-alpha.1`, CEL-to-Go conversions incorrectly restricted `int` / `uint` to 32-bit ranges on 64-bit targets. This affected function parameters, native literal construction, and map lookups; membership and optional access could silently report an existing key as absent. These conversions are fixed in [v0.1.0-alpha.2](../CHANGELOG.md#v010-alpha2). Users remaining on alpha.1 should use explicit `int64` / `uint64` in affected function signatures, map keys, and literal destination fields, or expose a typed projection with those types.
